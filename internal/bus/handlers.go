// Package bus wires the ollama inference engine onto the NATS observer bus via the shared
// cofiswarm-observer-sdk service component: it announces presence and serves the engine's
// .infer.ollama.* capability subjects (the bus equivalents of its HTTP /healthz and /v1/info).
package bus
...
const (
	SubjInfo   = servicecomponent.Prefix + ".infer.ollama.info"
	SubjHealth = servicecomponent.Prefix + ".infer.ollama.health"
)
...
func infoHandler(engine string) servicecomponent.Handler {
	return func([]byte) (any, error) {
		return infoReply{
			...
			Note: "run deploy/Dockerfile for full ollama-metal",
		}, nil
	}
}
