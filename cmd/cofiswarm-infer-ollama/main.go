...
func main() {
	...
	stopPresence := buspresence.StartPresence(os.Getenv("COFISWARM_BRIDGE_URL"), "infer-ollama", map[string]any{"name": "infer-ollama"})

	httpSrv := &http.Server{Addr: *addr, Handler: mux}
	go func() {
		log.Printf("infer-ollama metadata on %s", *addr)
		if err := httpSrv.ListenAndServe(); err != nil && err != http.ErrServerClosed {
			log.Fatalf("infer-ollama: server error: %v", err)
		}
	}()
	...
	defer stop()
	<-ctx.Done()
	log.Printf("infer-ollama: shutting down")
	stopPresence() // carrier goodbye -> offline
	shutCtx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	if err := httpSrv.Shutdown(shutCtx); err != nil {
		log.Printf("infer-ollama: graceful shutdown: %v", err)
	}
}
